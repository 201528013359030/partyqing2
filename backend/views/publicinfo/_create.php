...
//         id = 0;
//     }
//     $("#publicinfoform-receiver").text(text.substr(0,text.length-1));
//     $("#publicinfoform-receiverid").attr("value",id);
//     $("#publicinfoform-receivername").attr("value",text);
//     $("#publicinfoform-receiver").css("display","block");
//     return;
// }
// $(function(){
// 	//$("#publicinfoform-receiver").click(function(){
...
//     }else{
//         $("#publicinfoform-content").css("display","none");
...
//     $("#publicinfoform-receiverid").attr("value",id);
//     $("#publicinfoform-receivername").attr("value",text);
//     $("#publicinfoform-photo").attr("value",photo);
...
//         return false;
//     }
    $("#publicinfoform-contBox").css("display","none");
    $("#publicinfoform-tipBox").css("display","none");
    $("#publicinfoform-recBox").css("display","none");
    $("#publicinfoform-typeBox").css("display","none");
    if($("#publicinfoform-type").val()=='0'){
        $("#publicinfoform-typeBox").css("display","block");
        return false;
    }
    if( $.trim($("#publicinfoform-title").val()) == ""){
		$("#publicinfoform-tipBox").css("display","block");
		return false;
	}else{
    }
	if( $.trim($("#publicinfoform-content").val()) == "" && UE.getEditor('editor').getContent() == ""){
		$("#publicinfoform-contBox").css("display","block");
		return false;
	}else{
    }
    isSend = true;
...
<div id="wrap" class="wrap">
	...
	<div id="publicinfoform-typeBox" class="tipBox" style="display:none">		
		<div class="tipInner">
			<i class="ficon ic_wanning"></i>类型不能为空
		</div>
	</div>	
	<div id="publicinfoform-tipBox" class="tipBox" style="display:none">		
		<div class="tipInner">
			<i class="ficon ic_wanning"></i>标题不能为空
		</div>
	</div>	
	<div id="publicinfoform-contBox" class="tipBox" style="display:none">		
		<div class="tipInner">
    ...
    <form name="send_form" id="send_form" method="post" enctype="multipart/form-data" action="index.php?r=publicinfo/save">
	
	<div class="moGrid">

		<div class="formBox">
			<div class="inpBox">
				<select name="publicinfoForm[type]" id="publicinfoform-type">
				<option value='0'>-----请选择信息类型------</option>
				<?php foreach ($menus as $menu):?>
					<option value='<?=$menu->id?>' name="publicinfoForm[type]"><?=Html::encode($menu->menu_name)?></option>
				<?php endforeach;?>
				</select>
			</div>
			<div class="inpBox">
				<input id="publicinfoform-title" class="inp" type="text" name="publicinfoForm[title]" style="-webkit-tap-highlight-color:rgba(0,0,0,0);" value="" placeholder="标题" />
			</div>
			...
			<div class="inpBox">
				...
				<textarea id="publicinfoform-content" class="txtare" name="publicinfoForm[content]" style="-webkit-tap-highlight-color:rgba(0,0,0,0);height: 200px;display:none;" placeholder="公告正文..." onpropertychange="this.style.height=this.scrollHeight + 'px'" oninput="this.style.height=this.scrollHeight + 'px'"></textarea>
		            		
		        <div id=UEContent style="display:block">
			        <div>
			...
			<div id="uploadImg" class="inpBox" style="display:none;">
			...
			<div style="#display: none;">
				...
				<input id="publicinfoform-photo" type="hidden" name="publicinfoForm[photo]" value="">
				<input id="publicinfoform-filetype" type="hidden" name="publicinfoForm[filetype]" value="">
           
                <input id="publicinfoform-UE" type="hidden" name="publicinfoForm[UE]" value=0>
...
g('#J-del').onclick = function(){
	...
	g('#publicinfoform-contentImg').value = '';	
	g('#wrap').style.display = 'block';
	g('#J-showpic').style.display = 'none';
...
showpic();
//$("[name='publicinfoForm[top]']").bootstrapSwitch();
