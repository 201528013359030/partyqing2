...
miniSPA.changeUrl = function(){
	...
	var hashs = location.hash.replace("#","");//hash:获取url#号(锚)后面的字符串 --fyq
	...
	console.log(window[url].partial+'fff');
	//获取相应的本地html片段代码
	//请求一个页面，返回值是整个页面的代码 --fyq
	miniSPA.ajaxRequest(window[url].partial,"GET","HTML","","",function(status,page){
		...
		configs.divDemo.innerHTML = page; //将返回的页面发到main.php中 --fyq
