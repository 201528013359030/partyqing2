...
// 获取首页列表
function setMaterialListIteam(page,planid){
// var info
...
// alert(tpl_list);
	...
	// $(selector).html(content) html() 方法返回或设置被选元素的内容 (inner HTML)。
...
// return ;
	...
	for( var i=0; i<getList.length; i++ ){
		if(getList[i].video == "0"){
			display = "style=\"display: none;\"";
		}
		
		var html_iteam = tpl_list
			.replace( /\{id\}/g,getList[i].id )
...
// "url":SERVER_IP+"/question/web/index.php?r=test/info&id="+bankid,
	...
	if( /error/.test(getInfo) ){
		...
		console.log("查看资料内容getInfo报错："+getInfo);
		return false;
	}
...
function setCount(id,planid){
	...
	var arr = {
// "url":SERVER_IP+"/question/web/index.php?r=test/info&id="+bankid,
			"url":urlbase+"?r=edu/api&method=edu.plan.count",
			"arr":{"info":info},
			"type":"post"
	};
...
// return ;
	...
	for( var i=0; i<getList.length; i++ ){
		var src = /questionpaperadd/.test(getList[i].pic)?"../images/mr.png":getList[i].pic;// test():RegExpObject.test(string)=>如果字符串
																							// string
																							// 中含有与
																							// RegExpObject
																							// 匹配的文本，则返回
...
// var info ="{\"id\":\""+bankid+"\"}";
	...
	var arr = {
// "url":SERVER_IP+"/question/web/index.php?r=test/info&id="+bankid,
			"url":urlbase+"?r=leaquestion/api&method=Leaquestion.info.get",
			"arr":{"info":info},
			"type":"post"
	};
...
// "url":SERVER_IP+"/partyqing2/backend/web/index.php?r=leaquestion/api?method=Leaquestion.topic.get",
			"url":urlbase+"?r=leaquestion/api&method=Leaquestion.topic.get",
			"arr":{"info":info},
			"type":"post"
	};
// alert(JSON.stringify(arr));
	var getTopic = getJsonResult(arr);
...
// "url":SERVER_IP+"/question/web/index.php?r=test/submit",
			"url" : urlbase+"?r=leaquestion/api&method=Leaquestion.submit.get",
		"type":"post",
		"arr":{"info":info}
	};
	// console.log(arr);
	var sendQue = getJsonResult(arr);
...
// "url":SERVER_IP+"/question/web/index.php?r=test/end&paperid="+paperId
			"url" : urlbase +"?r=leaquestion/api&method=Leaquestion.end.get",
// "url" :
// "http://192.168.139.87/partyqing2/backend/web/index.php?r=leaquestion?method=Leaquestion.end.get",
			"arr" : {"info":info},
			"type":"post"
	};
	
	var getEnd = getJsonResult(arr);
...
// "url":SERVER_IP+"/question/web/index.php?r=test/false&bankId="+id
	...
	var getFalse = getJsonResult(arr);
