var commonFun = {
	...
	getJsonResult: function(method, arr,id) {
		...
		var URL = SERVER_IP + "/advanced/backend/web/index.php?r=eshop/api&method=" + method;
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.status == 0) {
					...
					var nolink = commonFun.dataEmptyError("nodata", "访问页面出错", "请返回重试");
					$("#"+id).html(nolink);
					console.log("获取" + method + "报错：" + data);
					return false;
				}
			},
...
getJsonResultBykeyvalue: function(method, str) {
...
getLocalTime:function(nS) {     
	...
	return new Date(parseInt(nS) * 1000).format('yyyy-M-d');
} 
