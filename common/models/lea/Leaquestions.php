 ...
 */
class Leaquestions  extends \common\models\WebService{
...
// 	header('Access-Control-Allow-Origin:*');
	...
	function __construct(){
		...
		parent::init();
        parent::registerApi("Leaquestion.list.get",
            "getLeaquestionlist",
            [
                "info"=>['type'=>'string'],
            ],
        ...
        parent::registerApi("Leaquestion.info.get",
        		"getLeaquestioninfo",
        		[
        				"info"=>['type'=>'string'],
        		],
        ...
        parent::registerApi("Leaquestion.topic.get",
        		"getLeaquestiontopic",
        		[
        				"info"=>['type'=>'string'],
        		],
        ...
        parent::registerApi("Leaquestion.submit.get",
        		"getLeaquestionsubmit",
        		[
        				"info"=>['type'=>'string'],
        		],
        		false
        ...
        parent::registerApi("Leaquestion.false.get",
        		"getLeaquestionfalse",
        		[
        				"info"=>['type'=>'string'],
        		],
        ...
        parent::registerApi("Leaquestion.top.get",
        		"getLeaquestiontop",
        		[
        				"info"=>['type'=>'string'],
        		],
        ...
        parent::registerApi("Leaquestion.end.get",
        		"getLeaquestionend",
        		[
        				"info"=>['type'=>'string'],
        		],
        ...
        parent::registerApi("Leaquestion.search.get",
        		"getLeaquestionsearch",
        		[
        				"info"=>['type'=>'string'],
        		],
        		false
...
//     	$paperid  = Yii::$app->request->get('paperid'); //试卷id
    	...
    	$correctNum = DjleaquestionAnswer::find()->where(['paperID'=>$paperid,'answerCorrect'=>0])->count();
    	...
    	$answerCount =   DjleaquestionAnswer::find()->where(['paperID'=>$paperid])->count();
...
//     	return $return;
    	...
    	$count = DjleaquestionList::find()->where(['bankID'=>$bankID])->groupBy(['uid'])->count();
    	//return $top;
    	foreach ($top as $key => $value) {
    		$userInfo = Memberlist::findOne(['guid'=>$value['uid']]); //返回值为数组类型
...
//     		$bankId  = Yii::$app->request->get('bankId');
    		...
    		$paperid = DjleaquestionList::find()->where(['bankID'=>$bankId,'finishflag'=>1])->max('paperid'); //选取最近抽到的试卷 --by fyq
    	}
    	$falseList = DjleaquestionAnswer::find()->where(['paperID'=>$paperid,'answerCorrect'=>1])->asArray()->all();
...
//     	$answer  = Yii::$app->request->post('answer'); //答案
    	...
    	$return['c'] = 0;
    	$return['m'] = '';
    	if(DjleaquestionAnswer::findOne(['questionid'=>$topicId,'paperID'=>$testId])){
    		$return['c'] = -3;
    		$return['m'] = '题目答案重复提交';
    		return $return;
    	}
    	...
    	$answerCount = DjleaquestionAnswer::find()->where(['paperID'=>$testId])->count();
    	...
    	if($answerCount >= $testInfo->questionNum){
    		$correctNum = DjleaquestionAnswer::find()->where(['paperID'=>$testId,'answerCorrect'=>0])->count();
    		$testInfo->correctNum = $correctNum;
    		$testInfo->falseNuM = $testInfo->questionNum-$correctNum;
    		$testInfo->finishflag = 1;
    		//    $testInfo->save();
...
//     	$answerList  = DjleaquestionList::find()->where(['bankID'=>$id])->groupBy(['uid'])->asArray()->all();
    	...
    	if($oid){
    		$list = DjleaquestionBank::find()->where(['deleteflag'=>0,'eid'=>$eid[1],'bankflag'=>'0','planid'=>$planid])->orWhere(['deleteflag'=>0,'eid'=>$eid[1],'orgid'=>$oid->officeAddress,'plainid'=>$planid])->limit($limit)->offset($offset)->asArray()->all();
    		$count = DjleaquestionBank::find()->where(['deleteflag'=>0,'eid'=>$eid[1],'bankflag'=>'0','planid'=>$planid])->orWhere(['deleteflag'=>0,'eid'=>$eid[1],'orgid'=>$oid->officeAddress,'plainid'=>$planid])->count();
    	}else{
    		$list = DjleaquestionBank::find()->where(['deleteflag'=>0,'eid'=>$eid[1],'bankflag'=>'0','planid'=>$planid])->limit($limit)->offset($offset)->asArray()->all();
...
//     	if($oid){
//     		$list = DjleaquestionBank::find()->where(['deleteflag'=>0,'eid'=>$eid[1],'bankflag'=>'0','planid'=>$planid])->andWhere(['or like',['title','bankid','creattime','sender','orgid','planid'],$searchtitle])->orWhere(['deleteflag'=>0,'eid'=>$eid[1],'orgid'=>$oid->officeAddress])->limit($limit)->asArray()->all();
//     		$count = DjleaquestionBank::find()->where(['deleteflag'=>0,'eid'=>$eid[1],'bankflag'=>'0','planid'=>$planid])->andWhere(['or like',['title','bankid','creattime','sender','orgid','planid'],$searchtitle])->orWhere(['deleteflag'=>0,'eid'=>$eid[1],'orgid'=>$oid->officeAddress])->count();
//     	}else{
//     		$list = DjleaquestionBank::find()->where(['deleteflag'=>0,'eid'=>$eid[1],'bankflag'=>'0','planid'=>$planid])->andWhere(['or like',['title','bankid','creattime','sender','orgid'],$searchtitle])->limit($limit)->asArray()->all();
//     		$count = DjleaquestionBank::find()->where(['deleteflag'=>0,'eid'=>$eid[1],'bankflag'=>'0','planid'=>$planid])->andWhere(['or like',['title','bankid','creattime','sender','orgid'],$searchtitle])->count();
//     	}
    	// 直接构造查询语句
...
//     	die($info);
    	...
    	if($answerList){
    		...
    		$topvalue=array();
    		foreach ($top as $key => $value) {
    			$userInfo = Memberlist::findOne(['guid'=>$value['uid']]);
    		...
    		//前五名
    		$data['top'] = ['title'=>'答题排行榜','name'=>implode(',',$topname),'value'=>implode(',',$topvalue)];
    		$myMax = DjleaquestionList::find()->where(['uid'=>$uid,'bankID'=>$id])->max('correctNum');
    		if(!$myMax){
    			$myMax = 0;
    		}
    		//查询得分比当前用户高的人数 -- by fyq
    		$ranking = DjleaquestionList::find()->where(['bankID'=>$id])->andWhere(['>', 'correctNum', $myMax])->groupBy(['uid'])->orderBy(['correctNum' => SORT_DESC])->count()+1;
    		//我的排名
    		...
    		$count = DjleaquestionList::find()->where(['uid'=>$uid,'bankID'=>$id,'finishflag'=>'1'])->count();
    	...
    	//是否有未完成
    	if(DjleaquestionList::findOne(['uid'=>$uid,'finishflag'=>0,'bankID'=>$id])){
    		$data['finishflag'] = 0;
    	}else{
    		$data['finishflag'] = 1;
    	}
...
//     	die($id);
    	...
    	if($new == 1){
    		// 重新答题
    		$test = DjleaquestionList::findOne(['uid'=>$uid,'finishflag'=>0,'bankID'=>$id]);
    		if($test){
    			if(!$test->delete()){
    				$return['c'] = -1;
    				$return['m'] = '重新开始失败';
    	...
    	$nowTest = DjleaquestionList::findOne(['uid'=>$uid,'finishflag'=>0,'bankID'=>$id]);
    	if(!$nowTest){
    		$bankInfo = DjleaquestionBank::findOne($id);
...
//     		die($bankInfo['questionnum']);
    	...
    	//获取当前题目
    	$count = DjleaquestionAnswer::find()->where(['paperID'=>$nowTest->paperid])->count();
